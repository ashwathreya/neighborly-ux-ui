...
export type SearchPageClientProps = {
	/** Query string from the server `searchParams` (stable per request). */
	initialSearchParams: Record<string, string>;
};

export function SearchPageClient({ initialSearchParams }: SearchPageClientProps) {
	const get = (key: string) => initialSearchParams[key] ?? '';
	...
	const [searchQuery, setSearchQuery] = useState({
		serviceType: String(get('serviceType') || get('petType') || 'all'),
		location: String(get('location') || ''),
		startDate: String(get('startDate') || ''),
		endDate: String(get('endDate') || ''),
		radius: String(get('radius') || '100'),
	});
	...
	const [mapRadius, setMapRadius] = useState<number>(() => parseFloat(get('radius') || '50') || 50);
